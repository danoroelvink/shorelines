function [CHANNEL,COAST] = move_channel(CHANNEL,COAST,TIME)
% function [CHANNEL,COAST] = move_channel(CHANNEL,COAST,TIME)
% 
% This function enforces channels to obtain a predefined width. 
% The width of the channels is monitored between two elements. 
% The model will push sediment in seaward direction along the perimeters 
% of the channel when the width is smaller than the equilibrium width. 
% The coastlines on both sides are adapted by moving sand seaward 
% according to the scheme dn -.5dn -.5dn. 
% Where dn is fac*(actual width - channelwidth) which is negative. 
% Note that two separate elements are needed, and not a single one. 
%
% INPUT:
%    CHANNEL
%       .used          : switch for the use of the channel function (0/1)
%       .xrmc          : x coordinates of rivers
%       .yrmc          : y coordinates of rivers
%       .channelwidth  : vector with minimum width(s) of river(s)
%       .fac           : factor (<1) applied to updating of coast sections
%       .x_inlet       : x coordinate of most seaward point of channel where width is less than channelwidth
%    COAST
%       .x_mc          : x coordinates of coastline sections;
...
% OUTPUT:
%    CHANNEL
%       .xrmc          : x coordinates of rivers (after update)
%       .yrmc          : y coordinates of rivers (after update)
%    COAST
%       .x_inlet       : x coordinates of the inlets (after update)
%       .y_inlet       : y coordinates of the inlets (after update)
%
%% Copyright notice
%   --------------------------------------------------------------------
...
%   --------------------------------------------------------------------
    ...
    if CHANNEL.used~=0 && ~isempty(CHANNEL.xrmc)
        plotdetails=0;
        ch='rgbm';
        section=get_sectionnr(COAST.x_mc); % for each point a section number is assigned
        %% Create grid along river axis
        
        [CHANNEL.xrmc,CHANNEL.yrmc]=initialize_grid(CHANNEL.xrmc,CHANNEL.yrmc,ds0);

        %% Center each river relative to the surrounding sections
        iriv=1;
        [ xr,yr,nr_mc,ir1(iriv),ir2(iriv) ] = get_one_polygon( CHANNEL.xrmc,CHANNEL.yrmc,iriv);
        ...
        %         xright=COAST.x_mc(d<0);yright=COAST.y_mc(d<0);
            ...
            [CHANNEL.xrmc,CHANNEL.yrmc]=insert_section(xrnew,yrnew,CHANNEL.xrmc,CHANNEL.yrmc,iriv);
            if plotdetails
        ...
        if plotdetails
            plot(CHANNEL.xrmc,CHANNEL.yrmc,'.k','linewidth',1)
