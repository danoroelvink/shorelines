...
%[QS,QSmax,adt,philoc,thetacrit]=transport2(phic,phiw,S,it,s);
%% Maximum long shore Transport
%[QSmax,adtmax,philoc,thetacrit]=transport2(phic,(phic-thetacrit),S,it,s);
%% Shadowing effect on Transport
%[shadowS,shadowS_h,shadow,philoc]=transport_shadow_treat(x,y,x_mc,y_mc,x_hard,y_hard,phiw,philoc);
...
%[philoc]=upwind_correction(philoc,n,cyclic,S,thetacrit,shadowS,shadowS_h);
       QS=S.b*S.Hso^2.5*sin(2*(philoc_cor));QScerc=QS;
QSmaxn=S.b*S.Hso^2.5*sin(2*(-45*pi/180));        
QSmaxp=S.b*S.Hso^2.5*sin(2*(45*pi/180));
...
%% Long shore Transport
% [QS,QSmax,adt,philoc,thetacrit]=transport2(phic,phiw,S,it,s,philoc);       
...
%     QS=S.b*S.Hso^2.5*sin(2*(philoc_cor));QScerc=QS;
    ...
    %thetacrit=45*pi/180;
    %QSmax=S.b*S.Hso^2.5*sin(2*(thetacrit));
end
