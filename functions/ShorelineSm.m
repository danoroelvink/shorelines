...
%S.dt=.02;                                                                  % time step [year] (use 1/365/4 for time-series)
...
if ~isfield(S,'timenum0')
    if ~isempty(S.reftime)
        timenum0 = datenum(S.reftime,'yyyy-mm-dd');
    else
        timenum0 = 0;
    end
else
...
if ~isempty(S.WVCfile)
    if S.RWSfiletype
        ...
        WVC=struct;
        WVC.timenum=datenum([num2str(WVCraw(:,1))],'yyyymmdd'); %,'%08.0f'),num2str(WVCraw(:,2),'%06.0f')
...
timenum1=timenum0;       
timenume = datenum(S.endofsimulation,'yyyy-mm-dd');

S.dt=0;
it=-1;   % for now = -1 to follow the exisiting code   


while  timenum1<timenume


 % timenum1 = timenum0 + (it)*S.dt*365;
 it=it+1;
timenum1 = timenum1+ S.dt*365; %dt [year]   %calculate the current time after each time step
...
timenuma(end+1)=timenum1;                              % add the new time to the time array
timenuma=unique(timenuma,'sorted');          %  sort and remove duplicates , to use in interpolation 
    
 WVC.timenumi=timenuma;  %follow the consequences 
    WVC.Hsi=interp1(WVC.timenum,WVC.Hs,timenuma); %If the interpolation needs more computational .. We could interpolate in between 
    WVC.Tpi=interp1(WVC.timenum,WVC.Tp,timenuma);
    WVC.Diri=mod(atan2(interp1(WVC.timenum,sin(WVC.Dir*pi/180),timenuma),interp1(WVC.timenum,cos(WVC.Dir*pi/180),timenuma))*180/pi,360);
    WVC.Hsi=interpNANs(WVC.Hsi);
    WVC.Tpi=interpNANs(WVC.Tpi);
    WVC.Diri=interpNANsDIR(WVC.Diri);
    %figure;plot(WVC.timenum,WVC.Dir,'b.',WVC.timenumi,WVC.Diri,'k.');
...
n_mc=length(find(isnan(x_mc)))+1;
    for i_mc=1:n_mc
        ...
        if strcmpi(S.trform,'CERC')
            ...
            %Smax=S.b*S.Hso^2.5;
         %adt=0.5*S.ds0^2/(S.b*S.Hso^2.5*cos(2*(phiw)))*S.d/(365*24*60*60) %should it be S.ds0 ?
         adt=0.5*S.ds0^2/(S.b*S.Hso^2.5*1)*S.d/(365*24*60*60) %assumption cos2deltb=1
        end
...
if ~isempty(S.WVCfile)
S.dt=min(adt,(min((WVC.timenum(it+2)-timenum1)/365,(timenume-timenum1)/365)));
else
    S.dt=min(adt,(timenume-timenum1)/365);  
end
    ...
    if mod(it,S.plotinterval)==0
        ...
        if 1
            ...
            if ~isempty(S.reftime)
                text(S.XYwave(1)-S.XYoffset(1),S.XYwave(2)-S.XYoffset(2)+diff(S.ylimits)/12,['H_{m0}=',num2str(S.Hso,'%2.1f'),' (',datestr(timenum1,'yyyy-mmm'),')']);
            else
                text(S.XYwave(1)-S.XYoffset(1),S.XYwave(2)-S.XYoffset(2)+diff(S.ylimits)/12,['H_{m0}=',num2str(S.Hso,'%2.1f'),' (',num2str(timenum1/365,'%3.2f'),' yr)']);
            end
            if 1
            ...
            if ~isempty(S.LDBplot)
                for mm=1:size(S.LDBplot,1)
                    ...
                    hp6(mm)=plot(LDBplotval(:,1)-S.XYoffset(1),LDBplotval(:,2)-S.XYoffset(2),S.LDBplot{mm,3});
                end
