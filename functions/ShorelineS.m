...
S.dt=.008;                                                                  % time step [year] (use 1/365/4 for time-series)
...
if ~isempty(S.WVCfile)
    if S.RWSfiletype
        ...
        WVC=struct;
        WVC.timenum=datenum([num2str(WVCraw(:,1))],'yyyymmddHH'); %,'%08.0f'),num2str(WVCraw(:,2),'%06.0f')
        %WVC.timenum=unique(WVC.timenum);
    ...
    for i_mc=1:n_mc
        ...
        if strcmpi(S.trform,'KAMP')
          ...
          %  QS = 365*24*60*60*(QSkampmass /(S.rhos-S.rhow)) /(1.0-S.porosity);QSkamp=QS;
            QS = 365*24*60*60*(QSkampmass *(S.rhos-S.rhow)) /S.rhos;QSkamp=QS;
    ...
    if mod(it,S.plotinterval)==0
        ...
        if 1
            ...
            if mod(it,figstep)==0
                ...
                fname=[num2str(round(timenum(it+1)+100000)),'.jpg'];
                setFIGUREproperties(800,600,32);
